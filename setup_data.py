...
with open(csv_path, "r") as f:
    ...
    attributes = header[1:]
    for row in reader:
        tmp_font_to_attribute_values[row[0]] = {
            a: v for a, v in zip(attributes, row[1:])
        }
...
for font_path in font_paths:
    ...
    if font_name in tmp_font_to_attribute_values.keys():
        valid_font_to_attribute_values[font_name] = tmp_font_to_attribute_values[
            font_name
        ]
...
validation_font_names = shuffled_font_names[
    int(len(shuffled_font_names) * 0.6) : int(len(shuffled_font_names) * 0.8)
]
test_font_names = shuffled_font_names[int(len(shuffled_font_names) * 0.8) :]
train_font_to_attribute_values = {
    font_name: all_font_to_attribute_values[font_name] for font_name in train_font_names
}
...
test_font_to_attribute_values = {
    font_name: all_font_to_attribute_values[font_name] for font_name in test_font_names
}

# save json files
json.dump(
    all_font_to_attribute_values,
    open("attributeData/all_font_to_attribute_values.json", "w"),
)
json.dump(
    train_font_to_attribute_values,
    open("attributeData/train_font_to_attribute_values.json", "w"),
)
json.dump(
    validation_font_to_attribute_values,
    open("attributeData/validation_font_to_attribute_values.json", "w"),
)
json.dump(
    test_font_to_attribute_values,
    open("attributeData/test_font_to_attribute_values.json", "w"),
)
...
url = "https://drive.google.com/uc?id=1Tym7rAIuaGr6Gv-gZRSJmPstQjOWPgl1"
file_path = "model_checkpoints/model.pt"
