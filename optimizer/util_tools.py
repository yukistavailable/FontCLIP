...
from optimizer.ttf_tools import font_string_to_svgs, normalize_letter_size
...
def preprocess(
    ...
    if font_path is None:
        ...
        # TODO: implement font_path
        # font_path = f"code/data/fonts/{font}.ttf"
    if init_path is None:
        raise NotImplementedError
        # TODO: prepare init_path
        # init_path = f"../svgs/init"
...
def combine_word(word, letter, font, experiment_dir, word_svg_scaled=None):
    if word_svg_scaled is None:
        word_svg_scaled = f"../svgs/init/{font}_{word}_scaled.svg"
    (
        canvas_width_word,
        canvas_height_word,
        shapes_word,
    ...
    w_min, w_max = (
        min([torch.min(shapes_word[ids].points[:, 0]) for ids in letter_ids]),
        max([torch.max(shapes_word[ids].points[:, 0]) for ids in letter_ids]),
    )
    h_min, h_max = (
        min([torch.min(shapes_word[ids].points[:, 1]) for ids in letter_ids]),
    ...
    out_w_min, out_w_max = (
        min([torch.min(p.points[:, 0]) for p in shapes]),
        max([torch.max(p.points[:, 0]) for p in shapes]),
    )
    out_h_min, out_h_max = (
        min([torch.min(p.points[:, 1]) for p in shapes]),
        max([torch.max(p.points[:, 1]) for p in shapes]),
    )
